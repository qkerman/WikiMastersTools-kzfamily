(() => {
  ...
  const { renderCardExtras } = featureRegistry.cardExtras.create({
    normalizeTitle,
    idByTitle,
    cardMetaById,
    readLocalValue,
  ...
  const { recordPullStats, renderPullStats } = featureRegistry.pullStats.create({
    RARITIES,
    PULL_STATS_KEY,
    readLocalValue,
    writeLocalValue,
  ...
  const { compactEligiblePage, applyCompactMode, ensureCompactControl } =
    featureRegistry.compactMode.create({
      COMPACT_MODE_KEY,
      readLocalValue,
      writeLocalValue,
      isCollectionPage,
  ...
  function handlePathChange() {
    ...
    if (isTradesPage()) resetTradesRequest();
